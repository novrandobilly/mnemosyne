...
export const useDrConfirmModal = () => {
  const navigate = useNavigate();
  const { showModal, closeModal } = useModal();

  const handleStart = () => {
    sessionStorage.removeItem("dr_progress");
    sessionStorage.removeItem("dr_seconds_left");
    navigate("/psikotes/dr/test-start");
    closeModal();
  };
        ...
        <div className="rounded-3xl border border-neutral-200 bg-white p-8 shadow-sm space-y-4">
          ...
          >
            ...
            <span className="font-semibold text-neutral-900">
              DR - Diagrammatic Reasoning
            </span>
            . Waktu pengerjaan adalah{" "}
            <span className="font-semibold text-neutral-900">20 menit</span>.
